<?php 
  ...
  if (isset($_GET['view'])) $view = sanitizerString($_GET['view']);
  else                      $view = $user;

  if (isset($_POST['text']))
  {
    $text = sanitizerString($_POST['text']);

    if ($text != "")
    {
      $pm   = substr(sanitizerString($_POST['pm']),0,1);
      $time = time();
...
_END;
    ...
    if (isset($_GET['erase']))
    {
      $erase = sanitizerString($_GET['erase']);
